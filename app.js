...
async function load() {
  try {
    const [v, e, p, r] = await Promise.all([
      sb
        .from("vehicles")
        .select("*")
    ...
    toast("Unable to load database: " + error.message);
  }
}
...
function renderDashboard() {
  const vehicleTotal = expenses.reduce(
    (a, e) => a + num(e.amount),
    0
  );
  ...
  if ($("nVehicles"))
    $("nVehicles").textContent = vehicles.length;
  ...
  if ($("nStorage"))
    $("nStorage").textContent =
      vehicles.filter(v => v.status === "Storage").length;

  if ($("nExpenses"))
    $("nExpenses").textContent = money(vehicleTotal);

  if ($("nPetty"))
    $("nPetty").textContent = money(pettyTotal);
...
function renderVehicles() {
  ...
  $("list").innerHTML = rows.length
    ? rows.map(card).join("")
    : `<p class="muted">
        No vehicles found. Use <b>+ Add Vehicle</b> to add one.
       </p>`;
}

function expenseTotalForVehicle(vehicleId) {
  return expenses
    .filter(e => String(e.vehicle_id) === String(vehicleId))
    .reduce((a, e) => a + num(e.amount), 0);
}

function categoryTotalForVehicle(vehicleId, category) {
  return expenses
    .filter(
      e =>
...
function card(v) {
  ...
  const parts = categoryTotalForVehicle(
    v.id,
    "Parts"
  );

  const materials = categoryTotalForVehicle(
    v.id,
    "Materials"
  );

  const labour = categoryTotalForVehicle(
    v.id,
    "Labour"
  );
      ...
      ${
        ...
        v.status === "Released"
          ...
          <div class="storage-summary">
            ...
            <div>
              Released to:
              ${esc(v.released_to || "Not yet released")}
            </div>

            <div>
              Contact:
              ${esc(v.released_contact || "Not provided")}
            </div>
          </div>
        `
          : ""
...
if ($("filter"))
  $("filter").onchange = renderVehicles;

// ======================================================
// VEHICLE BUTTONS
// ======================================================
if ($("addVehicle"))
  $("addVehicle").onclick = () => openVehicle();
...
if ($("quickExpense"))
  $("quickExpense").onclick = () => openExpense();

if ($("quickPetty"))
  $("quickPetty").onclick = () => openPetty();

if ($("quickReq"))
  $("quickReq").onclick = () => openReq();

if ($("quickPrint"))
  $("quickPrint").onclick = () => printSummary();
...
function updateStorageFields() {
  ...
  $("storageFields").classList.toggle(
    "hidden",
    !show
  );
}
...
if ($("status"))
  $("status").onchange = updateStorageFields;
...
if ($("vehicleForm")) {
  $("vehicleForm").onsubmit = async e => {
    ...
    if (!registration) {
      toast("Enter vehicle registration.");
      return;
    }
...
window.removeVehicle = async id => {
  ...
  // Delete requisitions for vehicle.
  const reqDelete = await sb
    .from("requisitions")
    .delete()
    .eq("vehicle_id", id);
...
function ensureExpenseCategory() {
  ...
  if (amount)
    ...
    $("expenseForm").appendChild(wrapper);
}
...
if ($("expenseForm")) {
  $("expenseForm").onsubmit = async e => {
    ...
    if (!vehicleId) {
      toast("Please select the vehicle.");
      return;
    }
    ...
    if (!description) {
      toast("Enter expense description.");
      return;
    }
...
function openPetty() {
  ...
  $("pettyModal")?.classList.remove(
    "hidden"
  );
}

if ($("pettyForm")) {
  $("pettyForm").onsubmit = async e => {
    e.preventDefault();
...
if ($("reqForm")) {
  $("reqForm").onsubmit = async e => {
    ...
    // --------------------------------------------------
    // FIRST: Try the proper "category" column.
    // --------------------------------------------------
    let result = await sb
      .from("requisitions")
      .insert({
    ...
    // --------------------------------------------------
    // If the database has "type" instead of category,
    // try that.
    // --------------------------------------------------
    if (
      result.error &&
    ...
    // --------------------------------------------------
    // If there is no category/type column at all,
    // preserve the type inside existing notes.
    // This allows your existing SQL structure to work.
    // --------------------------------------------------
    if (
      result.error &&
      (
    ...
    toast(
      `${category} requisition saved successfully.`
    );
  };
}

// ======================================================
// DETECT MISSING DATABASE COLUMN
// ======================================================
